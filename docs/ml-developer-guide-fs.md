...
* Feature computation modules under `ulta_mlops_demo/feature_engineering` folder. 
...
* Python unit tests for feature computation modules in `ulta_mlops_demo/tests/feature_engineering` folder.
* Feature engineering notebook, `ulta_mlops_demo/feature_engineering/notebooks/GenerateAndWriteFeatures.py`, that reads input dataframes, dynamically loads feature computation modules, executes their `compute_features_fn` method and writes the outputs to a Feature Store table (creating it if missing).
...
* An automated integration test is provided (in `.github/workflows/ulta_mlops_demo-run-tests-fs.yml`) that executes a multi task run on Databricks involving the feature engineering and model training notebooks.
...
1. Implement your feature module, address TODOs in `ulta_mlops_demo/feature_engineering/features` and create unit test in `ulta_mlops_demo/tests/feature_engineering`
2. Update `ulta_mlops_demo/resources/feature-engineering-workflow-resource.yml`. Fill in notebook parameters for `write_feature_table_job`.
3. Update training data path in `ulta_mlops_demo/resources/model-workflow-resource.yml`.

We expect most of the development to take place in the `ulta_mlops_demo/feature_engineering` folder.

## Iterating on ML code

### Deploy ML code and resources to dev workspace using Bundles

Refer to [Local development and dev workspace](../ulta_mlops_demo/resources/README.md#local-development-and-dev-workspace)
...
* Run `databricks configure --profile ulta_mlops_demo-dev --token --host <your-dev-workspace-url>`, passing the URL of your dev workspace.
...
* From within the root directory of the current project, use the [dbx sync](https://dbx.readthedocs.io/en/latest/guides/python/devloop/mixed/#using-dbx-sync-repo-for-local-to-repo-synchronization) tool to copy code files from your local machine into the Repo by running
  `dbx sync repo --profile ulta_mlops_demo-dev --source . --dest-repo your-repo-name`, where `your-repo-name` should be the last segment of the full repo name (`/Repos/username/your-repo-name`)

#### Running code on Databricks
You can iterate on ML code by running the provided `ulta_mlops_demo/feature_engineering/notebooks/GenerateAndWriteFeatures.py` notebook on Databricks using
...
#### Prerequisites
* Python 3.8+
* Install feature engineering code and test dependencies via `pip install -I -r ulta_mlops_demo/requirements.txt -r test-requirements.txt` from project root directory.
